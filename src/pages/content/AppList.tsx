...
export default () => {
  ...
  const pageConf = {
    total: total,
    onChange: (pageNo: number) => getAppList(pageNo),
  }
  ...
  const getAppList = async (pageNo: number = 1) => {
    const res: any = await http.get("app", { params: { pageNo } })
  ...
  const del = (_id: string) => {
    Modal.confirm({
      ...
      centered: true,
      async onOk() {
        const res: any = await http.delete(`app/${_id}`)
  ...
  const submit = async () => {
    try {
      ...
      optType === "add"
        ? await http.post("app", data)
        : await http.put(`app/${editId}`, data)
